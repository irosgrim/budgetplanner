...
class Donutchart extends Component {
    ...
    render() {
        ...
        const circleArr = JSON.parse(
            JSON.stringify(this.props.chartfrom.items)
        );
        let sections = [];
        //calculate section size for each item
        for (let key in circleArr) {
            if (circleArr.hasOwnProperty(key)) {
                circleArr[key].amount = this.handleSectionSize(
                    circleArr[key].amount
                );
                if (key > 0) {
                        ...
                        {sections.map((item, index) => {
                            return (
                                <li
                                    ...
                                    <div className="bold">
                                        {
                                            this.props.chartfrom.items[index]
                                                .amount
                                        }
                                    </div>
